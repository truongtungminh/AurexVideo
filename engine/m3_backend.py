...
def social_metadata(slug: str) -> dict:
    ...
    return {
        ...
        "instagramCaption": DEFAULT_FACEBOOK_CAPTION,
...
def character_cover_url(character_id: str, poses: list[dict]) -> str:
    """Return a stable preview asset for the character picker/dashboard.

    Video-based characters can ship a cached cover image (cover.jpg/png/webp)
    alongside their poses. Prefer that when present so the UI can keep using
    <img> previews without breaking on mp4 cover assets.
    """
    character_id = validate_character_id(character_id)
    ...
    for filename in (
        ...
        if (directory / filename).is_file():
            return f"/assets/characters/{character_id}/{filename}"
    first_file = Path(str((poses[0] if poses else {}).get("file") or "")).name
    if first_file:
        return f"/assets/characters/{character_id}/{first_file}"
...
def default_character_pose_timeline(
    character_id: str, pose_assets: dict[str, dict[str, str]], segments: list[dict],
) -> list[dict[str, object]]:
    """Assign a character's default pose cycle to each validated segment start."""
    sequence = default_pose_sequence(character_id, pose_assets)
    if not sequence:
        return []
    return [
        {"time": segment["start"], "pose": sequence[index % len(sequence)]}
        for index, segment in enumerate(segments)
    ]


def default_popsy_pose_timeline(
    pose_assets: dict[str, dict[str, str]], segments: list[dict],
) -> list[dict[str, object]]:
    """Assign the Popsy default pose cycle to each validated segment start."""
    return default_character_pose_timeline("popsy", pose_assets, segments)
    ...
    for path in sorted(CHARACTERS_ROOT.glob("*/manifest.json")):
        ...
        try:
            ...
            result.append({
                ...
                "coverUrl": character_cover_url(path.parent.name, poses),
            })
...
def save_character(payload: dict) -> dict:
    ...
    return {"character": {**manifest, "poseCount": len(poses), "coverUrl": character_cover_url(character_id, poses)}}
...
def update_character(current_id: str, payload: dict) -> dict:
    ...
    character = {
        **updated_manifest,
        ...
        "coverUrl": character_cover_url(next_id, updated_poses),
    }
...
def remembered_label_font(character_id: str) -> str:
    ...
    if character_id and isinstance(mappings, dict):
...
def remembered_label_color(character_id: str) -> str:
    defaults = read_project_defaults()
    mappings = defaults.get("labelColorByCharacter")
    if character_id and isinstance(mappings, dict):
        remembered = mappings.get(character_id)
        if isinstance(remembered, str) and remembered.strip():
            return normalize_hex_color(remembered, "#090909")
    return normalize_hex_color(defaults.get("labelColor"), "#090909")


def remembered_tts_voice(character_id: str) -> str:
    """Return the TTS voice id last used with this character, or '' if none."""
    defaults = read_project_defaults()
    mappings = defaults.get("ttsVoiceByCharacter")
    if isinstance(mappings, dict):
        remembered = mappings.get(str(character_id or "").strip())
        if remembered:
            return str(remembered).strip()
    return ""


def remember_tts_voice(character_id: str, voice_id: str) -> None:
    """Persist a TTS voice choice keyed by character id for later recall."""
    character_id = str(character_id or "").strip()
    voice_id = str(voice_id or "").strip()
    if not character_id or not voice_id:
        return
    defaults = read_project_defaults()
    mappings = defaults.get("ttsVoiceByCharacter")
    if not isinstance(mappings, dict):
        mappings = {}
    if mappings.get(character_id) == voice_id:
        return
    mappings[character_id] = voice_id
    defaults["ttsVoiceByCharacter"] = mappings
    CONFIG_ROOT.mkdir(parents=True, exist_ok=True)
    atomic_write_json(PROJECT_DEFAULTS_PATH, defaults)
...
def remember_project_defaults(slug: str, topic: dict) -> None:
    ...
    with PROJECT_DEFAULTS_LOCK:
        ...
        if character_id in ("engzy", "knowzy", "bietchichomet", "july"):
            ...
            # solid-white default are character-scoped, not global.
            for key in (
                "backgroundType",
                "backgroundColor",
                "backgroundImage",
        ...
        label_color = normalize_hex_color(
            topic.get("labelColor") or topic.get("leftLabelColor") or topic.get("rightLabelColor"),
            "#090909",
        )
        defaults["labelFontFamily"] = label_font
        defaults["labelColor"] = label_color
        if character_id:
            ...
            color_mappings = defaults.get("labelColorByCharacter")
            if not isinstance(color_mappings, dict):
                color_mappings = {}
            color_mappings[character_id] = label_color
            defaults["labelColorByCharacter"] = color_mappings
...
def normalize_tts_config(value: object, fallback: object = None) -> dict:
    """Keep project-level Maziao speaker definitions safe and portable."""
    raw = value if isinstance(value, dict) else fallback if isinstance(fallback, dict) else {}
    raw_mode = str(raw.get("mode") or raw.get("ttsMode") or raw.get("tts_mode") or "multiSpeakers").strip().lower()
    if raw_mode in {"multispeaker", "multispeakers"}:
        mode = "multiSpeakers"
    elif raw_mode in {"paragraph", "auto"}:
        mode = raw_mode
    else:
        mode = "multiSpeakers"
    ...
    if isinstance(raw_speakers, list):
        for index, item in enumerate(raw_speakers[:32], 1):
            if isinstance(item, dict):
                entries.append((str(item.get("id") or item.get("key") or item.get("alias") or item.get("name") or f"speaker-{index}"), item))
    elif isinstance(raw_speakers, dict):
        entries = [(str(key), item) for key, item in list(raw_speakers.items())[:32]]

    speakers: dict[str, dict] = {}
    for raw_key, raw_value in entries:
        key = re.sub(r"[^a-zA-Z0-9_-]+", "_", str(raw_key).strip()).strip("_")[:80]
        if not key:
            continue
        item = {"voiceId": raw_value} if isinstance(raw_value, str) else dict(raw_value) if isinstance(raw_value, dict) else {}
        cleaned: dict[str, object] = {
            "alias": str(item.get("alias") or item.get("name") or raw_key).strip()[:80] or key,
            "voiceId": str(item.get("voiceId") or item.get("voice_id") or item.get("voice") or "oncoinx").strip()[:200] or "oncoinx",
        }
        for field, minimum, maximum in (("speed", 0.4, 1.6), ("pitch", 0.6, 1.4)):
            try:
                cleaned[field] = round(max(minimum, min(maximum, float(item.get(field, 1.0)))), 3)
            except (TypeError, ValueError):
                cleaned[field] = 1.0
        speakers[key] = cleaned
    normalized = {"mode": mode, "speakers": speakers}
...
def normalize_custom_slides(raw: object, segment_count: int) -> list[dict]:
    ...
    for index, candidate in enumerate(raw if isinstance(raw, list) else []):
        ...
        for raw_layer in (candidate.get("layers") or [])[:12]:
            ...
            default_y = 20 if layer_type == "text" else 0
            default_h = 12 if layer_type == "text" else 100
            raw_x = raw_layer.get("x")
            raw_y = raw_layer.get("y")
            raw_w = raw_layer.get("w")
            raw_h = raw_layer.get("h")
            if layer_type == "image" and (
                (isinstance(raw_x, (int, float)) and isinstance(raw_y, (int, float))
                 and isinstance(raw_w, (int, float)) and isinstance(raw_h, (int, float))
                 and abs(float(raw_x) - 6) < 0.01 and abs(float(raw_y) - 34) < 0.01
                 and abs(float(raw_w) - 88) < 0.01 and abs(float(raw_h) - 38) < 0.01)
                or (isinstance(raw_x, (int, float)) and isinstance(raw_y, (int, float))
                    and isinstance(raw_w, (int, float)) and isinstance(raw_h, (int, float))
                    and abs(float(raw_x)) < 0.01 and abs(float(raw_y) - 34) < 0.01
                    and abs(float(raw_w) - 100) < 0.01 and abs(float(raw_h) - 32) < 0.01)
            ):
                raw_x, raw_y, raw_w, raw_h = 0, 0, 100, 100
            layer = {
                "id": layer_id or _custom_token(layer_type), "type": layer_type,
                "x": _clamp_custom_number(raw_x, 0, 0, 92),
                "y": _clamp_custom_number(raw_y, default_y, 0, 100 if layer_type == "image" else 92),
                "w": _clamp_custom_number(raw_w, 100, 8, 100),
                "h": _clamp_custom_number(raw_h, default_h, 6, 100),
            }
...
def normalize_topic(slug: str, payload: dict) -> dict:
    ...
    for item in segments[:200]:
        ...
        cleaned_segment = {"start": round(start, 3), "end": round(end, 3), "text": text}
        speaker = str(
            item.get("speaker")
        ...
        if speaker:
            cleaned_segment["speaker"] = speaker[:80]
        if alias:
            cleaned_segment["alias"] = alias[:80]
        if voice_id:
            cleaned_segment["voiceId"] = voice_id[:200]
        for key, minimum, maximum in (("speed", 0.4, 1.6), ("pitch", 0.6, 1.4)):
            if item.get(key) is None:
                continue
            try:
                cleaned_segment[key] = round(max(minimum, min(maximum, float(item[key]))), 3)
            except (TypeError, ValueError):
                raise ValueError(f"{key} của segment phải là số.")
    ...
    if topic["projectType"] == "quiz":
        if "quizItems" in payload or "quiz_items" in payload:
            ...
            options_count = 1 if is_picture_quiz else THEGIOIDODAY_OPTIONS_COUNT if brand_id == "thegioidoday" else 3
        ...
        if topic.get("quizTemplate") == PICTURE_QUIZ_TEMPLATE:
            ...
            if script_lines:
                topic["quizScriptLines"] = script_lines[:PICTURE_QUIZ_ITEM_COUNT * 2]
    ...
    topic["labelColor"] = normalize_hex_color(
        payload.get("labelColor", current.get("labelColor", remembered_label_color(str(payload.get("characterId") or current.get("characterId") or "")))),
        "#090909",
    )
    ...
    if requested_character_id in {"popsy", "bietchichomet"} and "poseTimeline" not in payload:
        # Headless clients may update segments without a poseTimeline. Build a
        # fresh character cycle from those segment starts instead of retaining
        # a stale timeline from the current topic.
        timeline = default_character_pose_timeline(requested_character_id, pose_assets, cleaned_segments)
    else:
    ...
    topic["sfxVolume"] = round(max(0.0, min(1.0, float(payload.get("sfxVolume", current.get("sfxVolume", 0.3))))), 2)
...
def save_topic(slug: str, payload: dict) -> dict:
    ...
    if script_source and topic.get("quizCtaText") and topic.get("quizTemplate") != PICTURE_QUIZ_TEMPLATE:
...
def parse_picture_quiz_script(value: object, language: object = "en", item_count: int = PICTURE_QUIZ_ITEM_COUNT) -> tuple[list[str], list[dict[str, object]], list[str]]:
    """Parse image vocabulary quiz as question/answer pairs.

    Older five-line Picture Quiz scripts are still accepted, then collapsed to
    the spoken two-line contract used by the current template.
    """
    lines = _quiz_script_lines(value)
    pair_line_count = item_count * 2
    old_line_count = item_count * QUIZ_LINES_PER_ITEM
    if len(lines) >= old_line_count:
        display_lines = lines[:old_line_count]
        use_old_format = True
    elif len(lines) >= pair_line_count:
        display_lines = lines[:pair_line_count]
        use_old_format = False
    else:
        raise ValueError(f"Picture Quiz cần đủ {item_count} cặp câu hỏi và đáp án.")
    spoken_lines: list[str] = []
    raw_items: list[dict[str, object]] = []
    for item_index in range(item_count):
        offset = item_index * (QUIZ_LINES_PER_ITEM if use_old_format else 2)
        ...
        if use_old_format:
            ...
            answer_text = answer_match.group(2).strip().rstrip(".") if answer_match else ""
        else:
            answer_line = display_lines[offset + 1]
            answer_match = PICTURE_QUIZ_ANSWER_RE.search(answer_line)
            answer_text = (answer_match.group(2) if answer_match else answer_line).strip().rstrip(".")
            options = [answer_text]
        if not question or any(not option for option in options) or not answer_text:
            raise ValueError(f"Câu Picture Quiz {item_index + 1}: kịch bản không hợp lệ.")
        answer_letter = ((answer_match.group(1) if answer_match else "") or "").upper()
        if answer_letter:
            correct_index = ord(answer_letter) - ord("A")
            if correct_index < 0 or correct_index >= len(options):
                raise ValueError(f"Câu Picture Quiz {item_index + 1}: đáp án phải khớp lựa chọn.")
        else:
        ...
        raw_items.append({"question": question, "options": options, "correct_index": correct_index})
    return spoken_lines, normalize_quiz_items(raw_items, item_count=item_count, options_count=1), spoken_lines
...
def normalize_quiz_items(value: object, item_count: int = QUIZ_ITEM_COUNT, options_count: int = 3) -> list[dict[str, object]]:
    ...
    for index, raw in enumerate(value, 1):
        ...
        flexible_picture_options = options_count == 1
        if not question or not isinstance(options, list) or (len(options) < 1 if flexible_picture_options else len(options) != options_count):
        ...
        if correct_index not in set(range(len(cleaned_options))):
            raise ValueError(f"correct_index của câu Quiz {index} phải nằm trong 0..{len(cleaned_options) - 1}.")
...
<text x="400" y="500" text-anchor="middle" fill="#4b3a29" font-family="Arial,sans-serif" font-size="42" font-weight="700">{label}</text>
    ...
    if background_type not in {"default", "color", "image"}:
        background_type = "default"
    # engzy/july's pose videos already contain the complete vertical scene, and
    # bietchichomet renders on a transparent stage. Do not copy the shared
    # background default into newly created projects for these characters.
    if character_id in ("engzy", "bietchichomet", "july"):
        background_type = "default"
    background_color = normalize_hex_color(defaults.get("backgroundColor"), "#f5eee3")
    # knowzy always renders on a plain solid white background instead of the
    # shared image preset; this default is character-scoped, not global.
    if character_id == "knowzy":
        background_type = "color"
        background_color = "#ffffff"
    # Character-scoped solid background default (config-driven via
    # project-defaults.json -> backgroundColorByCharacter, e.g.
    # anhtinhbiettuot -> #f3ebdd) overrides the shared preset.
    scoped_backgrounds = defaults.get("backgroundColorByCharacter")
    if isinstance(scoped_backgrounds, dict):
        scoped_bg = scoped_backgrounds.get(character_id)
        if scoped_bg:
            background_type = "color"
            background_color = normalize_hex_color(scoped_bg, background_color)
    try:
    ...
    background_image = ""
    if background_type == "image" and character_id != "engzy":
    ...
    # Character-scoped karaoke colours (project-defaults.json ->
    # karaokeColorByCharacter / karaokeActiveColorByCharacter).
    karaoke_scoped = defaults.get("karaokeColorByCharacter")
    karaoke_scoped = karaoke_scoped if isinstance(karaoke_scoped, dict) else {}
    karaoke_active_scoped = defaults.get("karaokeActiveColorByCharacter")
    karaoke_active_scoped = karaoke_active_scoped if isinstance(karaoke_active_scoped, dict) else {}
    ...
    topic = {
        ...
        "labelColor": remembered_label_color(character_id),
    ...
    if quiz_cta_text and quiz_template != PICTURE_QUIZ_TEMPLATE:
...
def audio_source_for_job(job_id: str, slug: str, options: dict) -> Path:
    ...
    if source == "elevenlabs":
        ...
        if tts_extra:
            command.extend(["--tts-config-json", json.dumps(tts_extra, ensure_ascii=False)])
    else:
...
def run_job(job_id: str) -> None:
    ...
    try:
        ...
        # Fix: lưu luôn bản đã co giãn theo duration audio thực vào topic.json
        # để editor và các lần load sau không còn giữ timeline 60s placeholder.
        atomic_write_json(project_dir(slug) / "topic.json", aligned_topic)
