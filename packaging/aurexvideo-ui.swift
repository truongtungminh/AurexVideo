...
final class AppState: ObservableObject {
    ...
    func stopServer() {
        ...
        if let pid = serverProcessPID {
            ...
            try? proc.run()
            proc.waitUntilExit()
            serverProcessPID = nil
        }
        let cleanup = Process()
        cleanup.executableURL = URL(fileURLWithPath: "/bin/sh")
        cleanup.arguments = ["-c", "lsof -ti tcp:4173 2>/dev/null | xargs -r kill -9 2>/dev/null"]
        try? cleanup.run()
        cleanup.waitUntilExit()
        if fm.fileExists(atPath: engineBase.appendingPathComponent("app.log").path) {
            try? ("[stopServer] requested cleanup of port 4173".write(to: engineBase.appendingPathComponent("app.log"), atomically: true, encoding: .utf8))
        }
    }

    @discardableResult
    func ensurePort4173Free(maxAttempts: Int = 10) -> Bool {
        let logURL = engineBase.appendingPathComponent("app.log")
        for attempt in 1...maxAttempts {
            let cleanup = Process()
            cleanup.executableURL = URL(fileURLWithPath: "/bin/sh")
            cleanup.arguments = ["-c", "lsof -ti tcp:4173 2>/dev/null | xargs -r kill -9 2>/dev/null || true"]
            try? cleanup.run()
            cleanup.waitUntilExit()

            let checker = Process()
            checker.executableURL = URL(fileURLWithPath: "/bin/sh")
            checker.arguments = ["-c", "lsof -ti tcp:4173 2>/dev/null | wc -l | tr -d ' '"]
            let pipe = Pipe()
            checker.standardOutput = pipe
            try? checker.run()
            checker.waitUntilExit()
            let data = pipe.fileHandleForReading.readDataToEndOfFile()
            let count = Int(String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "0") ?? 0
            if count == 0 {
                try? ("[ensurePort4173Free] port freed on attempt \(attempt)".write(to: logURL, atomically: true, encoding: .utf8))
                return true
            }
            usleep(300_000)
        }
        try? ("[ensurePort4173Free] port 4173 still busy after \(maxAttempts) attempts".write(to: logURL, atomically: true, encoding: .utf8))
        return false
    }
    ...
    func startServerAndShow() {
        ...
        guard ensurePort4173Free() else {
            let m = "Port 4173 vẫn đang bị chiếm. Hãy đóng app cũ rồi mở lại."
