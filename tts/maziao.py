...
VN_VOICES: dict[str, dict] = {
    ...
    "giaovien1": {
        "id": "clone_kDJZaNoawkeT4ghTsIylG",
        "name": "giaovien1",
        "gender": "female",
        "modelId": "vieten_speech",
    },
    "giaovien": {
        "id": "clone_miFp2KSKIISRrDwAw0q1b",
        "name": "giaovien",
        "gender": "female",
        "modelId": "vieten_speech",
    },
    "hocsinhnu": {
        "id": "clone_JLr80anLvVt6lLBKTr5ZQ",
        "name": "hocsinhnu",
        "gender": "female",
        "modelId": "vieten_speech",
    },
    "better-version": {
        "id": "cPJSNWzr4yUDpR5Is4M4p",
        "name": "Better Version",
        "gender": "female",
        "modelId": "vieten_speech",
    },
    "tintucbitcoin": {
        "id": "clone_OrG54aYf_E8CgwVw9kgbu",
        "name": "tintucbitcoin",
        "gender": "female",
        "modelId": "vieten_speech",
    },
    "co-gai-hoat-ngon": {
        "id": "PE_ETGOrjDNmlIqKUwqqd",
        "name": "Cô gái Hoạt ngôn",
        "gender": "female",
        "modelId": "vieten_speech",
    },
    "ethan": {
        "id": "0KdW9bPh6HNMCxJV1GXF0",
        "name": "Ethan",
        "gender": "male",
        "language": "en",
...
def normalize_tts_mode(value: object, default: str = "paragraph") -> str:
    """Return the canonical Maziao mode used by the render pipeline."""
    raw = re.sub(r"[^a-z]", "", str(value or "").strip().lower())
    if raw in {"", "auto"}:
        raw = re.sub(r"[^a-z]", "", str(default or "paragraph").strip().lower())
    if raw in {"multispeaker", "multispeakers"}:
        return "multiSpeakers"
    if raw in {"paragraph", "text"}:
        return "paragraph"
    raise ValueError("Maziao TTS mode phải là paragraph hoặc multiSpeakers.")
...
def _speaker_config_entries(raw_config: object) -> dict[str, dict]:
    """Normalize speaker definitions keyed by id, alias, or name."""
    if isinstance(raw_config, dict) and isinstance(raw_config.get("speakers"), (dict, list)):
        raw_config = raw_config["speakers"]

    if isinstance(raw_config, dict) and any(key in raw_config for key in ("voiceId", "voice_id", "voice")):
        raw_config = {"default": raw_config}

    if isinstance(raw_config, list):
        items = []
        for index, value in enumerate(raw_config):
            if isinstance(value, dict):
                key = value.get("id") or value.get("key") or value.get("alias") or value.get("name") or f"speaker-{index + 1}"
                items.append((str(key), value))
    elif isinstance(raw_config, dict):
        items = list(raw_config.items())
    else:
        items = []

    entries: dict[str, dict] = {}
    for key, value in items:
        config = {"voiceId": value} if isinstance(value, str) else (dict(value) if isinstance(value, dict) else {})
        if not config:
            continue
        alias = str(config.get("alias") or config.get("name") or key or "").strip()
        voice = str(config.get("voiceId") or config.get("voice_id") or config.get("voice") or "").strip()
        normalized = {
            "alias": alias[:MAX_MULTISPEAKER_ALIAS_LENGTH],
            "voiceId": voice,
            "speed": config.get("speed"),
            "pitch": config.get("pitch"),
        }
        for candidate in (key, alias, config.get("id"), config.get("key"), config.get("name")):
            candidate_key = str(candidate or "").strip().casefold()
            if candidate_key:
                entries[candidate_key] = normalized
    return entries


def build_maziao_multispeaker_parts(
    segments: list[dict] | None,
    default_voice: str | None = None,
    *,
    speaker_config: object = None,
    default_speed: float = 1.0,
    default_pitch: float = 1.0,
) -> list[dict]:
    """Build Maziao ``multiSpeakers`` parts from project segments."""
    fallback_voice_id, _ = _resolve_voice(default_voice)
    entries = _speaker_config_entries(speaker_config)
    source_segments = segments if isinstance(segments, list) else []

    parts: list[dict] = []
    marker_pattern = re.compile(r"^\[([^\]]+)\]\s*(.*)$", re.DOTALL)
    for segment in source_segments:
        ...
        if marker_match and not marker_match.group(1).strip().casefold().startswith("delay="):
            marker_alias, marker_text = marker_match.groups()
            if not speaker_ref:
                speaker_ref = marker_alias.strip()
            text = marker_text.strip()
        if not text:
            continue

        speaker = entries.get(speaker_ref.casefold()) if speaker_ref else None
        explicit_voice = str(
            segment.get("voiceId")
            or segment.get("voice_id")
            or segment.get("voice")
            or ""
        ).strip()
        voice_raw = explicit_voice or str((speaker or {}).get("voiceId") or "").strip()
        if voice_raw:
            voice_id, _ = _resolve_voice(voice_raw)
        else:
            voice_id = fallback_voice_id

        alias = str(segment.get("alias") or (speaker or {}).get("alias") or speaker_ref or "").strip()
        part = {
            "text": text,
            "voiceId": voice_id,
            "speed": _bounded_float(
                segment.get("speed", (speaker or {}).get("speed", default_speed)),
                default_speed,
                0.4,
                1.6,
            ),
            "pitch": _bounded_float(
                segment.get("pitch", (speaker or {}).get("pitch", default_pitch)),
                default_pitch,
                0.6,
                1.4,
            ),
        ...
        if alias:
            part["alias"] = alias[:MAX_MULTISPEAKER_ALIAS_LENGTH]
        parts.append(part)

    if not parts:
        raise ValueError("Multi-speakers cần ít nhất một segment có text.")
    if len(parts) > MAX_MULTISPEAKER_PARTS:
        raise ValueError(f"Multi-speakers tối đa {MAX_MULTISPEAKER_PARTS} segment.")
    return parts


def _submit_payload(payload: dict, api_base: str, headers: dict) -> dict:
    """Submit a Maziao TTS payload and return its task metadata."""
...
def _submit_tts(
    ...
    return _submit_payload(payload, api_base, headers)


def _submit_multispeakers_tts(
    parts: list[dict],
    model_id: str,
    api_base: str,
    headers: dict,
    ...
    preview_text = "\n".join(
        f"[{part['alias']}] {part['text']}" if part.get("alias") else str(part["text"])
        for part in parts
    )
    payload = {
        "mode": "multiSpeakers",
...
async def generate_maziao_full_audio(
    ...
    *,
    ...
    voice_id, resolved_model_id = _resolve_voice(voice)
    model_id = str(model_id or resolved_model_id or DEFAULT_MODEL_ID).strip()
...
async def generate_maziao_multispeakers_audio(
    slide_dir: Path,
    output_dir: Path,
    lines: list[str],
    *,
    ...
    """Generate one full voiceover using Maziao's ``multiSpeakers`` mode."""
    del slide_dir
    resolved_speed = normalize_speed(speed)
    _, resolved_base, headers = _resolve_api_config(api_key, api_base)
    voice_id, resolved_model_id = _resolve_voice(voice)
    model_id = str(model_id or resolved_model_id or DEFAULT_MODEL_ID).strip()
    source_segments = segments if isinstance(segments, list) else [
        {"text": line} for line in lines if str(line or "").strip()
    ]
    parts = build_maziao_multispeaker_parts(
        source_segments,
        voice_id,
    ...
    if len(text) < MIN_TEXT_LENGTH:
        raise ValueError(
            f"Maziao requires text > {MIN_TEXT_LENGTH} chars (got {len(text)}). "
            "Use a longer script or add more dialogue segments."
        )
    if len(text) > MAX_TEXT_LENGTH:
    ...
    if meta_file.exists():
        try:
            ...
            cache_matches = False
    if not force and cache_matches and audio_file.exists() and audio_file.stat().st_size > 0:
        print(f"Multi-speakers Maziao TTS: {audio_file} (cached)")
        return audio_file

    if audio_file.exists():
        audio_file.unlink()
    speaker_labels = ", ".join(
        str(part.get("alias") or part.get("voiceId") or "speaker")
        for part in parts
    )
    print(
        f"Voice: Maziao multi-speakers ({speaker_labels}; model: {model_id}, "
        f"speed {resolved_speed:g}x)"
    )
    print(f"Submitting multiSpeakers script: {len(parts)} parts, {len(text)} chars")

    loop = asyncio.get_running_loop()
    submit_result = await loop.run_in_executor(
        None, _submit_multispeakers_tts, parts, model_id, resolved_base, headers,
    )
    task_id = submit_result["taskId"]
    cost = submit_result.get("totalCost", "?")
    remaining = submit_result.get("remainingCredits", "?")
    ...
    meta_file.write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Multi-speakers Maziao TTS saved: {audio_file}")
    return audio_file
